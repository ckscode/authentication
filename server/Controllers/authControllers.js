      ...
      const emailData = {
        ...
        html: `
         ...
         if(updateResetLink.acknowledged !== true){
            return res.status(400).json({error:'Database connection error for forget password request'})
         }
      sendEmail(req, res, emailData);
...
export const resetPassword = async(req,res) =>{
      try{
         ...
         if(resetPasswordLink){
            jwt.verify(resetPasswordLink,process.env.JWT_RESET_PASSWORD,function(err,data){
                if(err){
                    return res.status(401).json({
...
export const googleLogin = async(req,res) =>{
    try{
        const {data} =req.body;
      const clientData= await client.verifyIdToken({idToken:data,audience:process.env.GOOGLE_CLIENT_ID})
       .then(response =>{console.log("GOOGLE LOGIN RESPONSE",response.payload);return response.payload}).catch(err=>console.log(err))
